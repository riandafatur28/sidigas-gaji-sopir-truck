...
@section('title', 'LLM Parser Ritase')

@section('content')
<div class="max-w-4xl mx-auto py-6">
    <div class="mb-6 flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">AI/LLM Parser Ritase</h1>
            <p class="text-gray-600 mt-1">Paste teks jadwal sopir (format grup WA). <strong>Dual-mode:</strong> LLM AI + Rule-based fallback.</p>
        </div>
        <a href="{{ route('ritase.parser') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300">
            Parser Konvensional →
        </a>
    </div>
            ...
            <div class="bg-white shadow rounded-lg">
                <div class="p-6 border-b border-gray-200">
                    <div class="flex items-center gap-2 mb-2">
                        <span class="px-2 py-1 text-xs font-semibold rounded bg-purple-100 text-purple-800">🤖 LLM</span>
                        <span class="px-2 py-1 text-xs font-semibold rounded bg-green-100 text-green-800">⬇️ Fallback</span>
                        <span class="px-2 py-1 text-xs font-semibold rounded bg-yellow-100 text-yellow-800">🎯 Confidence</span>
                    </div>
                    <p class="text-sm text-gray-600">AI coba parse dulu. Kalau gagal (401/rate limit), auto turun ke rule-based.</p>
                </div>

                <form method="POST" action="{{ route('ritase.llm.process') }}" class="p-6 space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <textarea name="text" id="text" rows="15"
                            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 font-mono text-sm"
                    ...
                    <div>
                        ...
                        <select name="periode_id" id="periode_id" required
                            class="w-full max-w-xs px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
            ...
            <div class="bg-white shadow rounded-lg p-4">
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex items-center gap-2">
                        <span class="px-2 py-0.5 text-xs rounded bg-purple-100 text-purple-800">LLM</span>
                        <span>AI-powered extraction</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <span class="px-2 py-0.5 text-xs rounded bg-green-100 text-green-800">Rule</span>
                        <span>Keyword-based fallback</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <span class="px-2 py-0.5 text-xs rounded bg-yellow-100 text-yellow-800">Conf</span>
                        <span>Confidence score + hallucination check</span>
                    </div>
                </div>
            </div>
